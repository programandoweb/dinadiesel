...
function MakeEstadoTarea($name,$estado=null,$extra=array()){
	$options = array(
		...
		'2'         => 'Terminado',
		'1'         => 'En proceso',
		'0'       	=> 'En espera de asignación'
	);
	return form_dropdown($name, $options, $estado,$extra);
}

function MakeEstadoPrioridad($name,$estado=null,$extra=array()){
	$options = array(
		'3'         => 'Alta',
		'2'         => 'Media',
		'1'         => 'Baja',
...
function makeMarcas($name,$estado=null,$return=false){
	$ci=&get_instance();
	$tabla			=	DB_PREFIJO."maestro_marcas";
	$ci	->db->select("*")->from($tabla);
...
function makeModelo($name,$estado=null,$return=false){
	$ci=&get_instance();
